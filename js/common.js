...
var guiderImg_Arr = []; //存储封面图 imgURL
...
function checkPic(files) {
    ...
    fileList.forEach((item) => {
        ...
        // }
        // 判断操作的是否为封面图
        if($(fileDom).attr('data_type') === 'card_4'){
            // 先存一下即将丢失的item
            guiderImg_item = item;
            //裁剪图片(此时图片格式为base64)                
            cropGuiderImg(localShowImg(item))
        }else{
            //本地展示图片            
            fileDom.nextElementSibling.src = localShowImg(item);
            //图片压缩并转为file文件        
...
// 专门处理封面图
function cropGuiderImg(imgurl){    
    // 显示遮罩层
    vm.showCropper = true
    vm.uncropGuiderImg_imgurl = imgurl
    setTimeout(function(){
        //图片裁剪                 
        $('.cropImg').cropper({
            aspectRatio: 4 / 3,
...
//保存封面并进行上传前的压缩处理，然后立即上传
function save_imgURL(){
    // 获取base64格式的图片路径
    ...
    // 1本地回显裁剪后的图
    vm.guiderImg_imgURL = base64url
    // 2开始压缩、上传裁剪后的图
    var img = new Image()
    var fileType = guiderImg_item.type
    var quality = 10; //图片品质1-100
    img.src = base64url //base64url
...
//                alert("src:--"+src)
        img.src = src;
        _blob = dataURItoBlob(src); //转二进制,转完后放置为待上传的文件           
//          file.filename = Math.round(Math.random() * 100000000000000, 0) + ".jpg";
        // 调上传接口
        // uploadCos(file)
        upload(cosParamObj, guiderImg_item);
    }, 1)
}
// 上传cos前的压缩处理
function begin(item){
...
//COS 上传功能 封面图
function _sliceUploadFile_4(obj1, file) { // _4 封面图
    let obj = vm.cosParamObj;
    //自定义key的格式
    var _key = set_COSImgKey(file);
    //console.log(_key);
    ...
    cos.sliceUploadFile({
        ...
        if(!err && data){
            cos.sliceUploadFile({
                ...
                if (data.statusCode == 200) { // 上传成功返回url
                    //截取url
                    var _i = data.Location.lastIndexOf('guiderImg/');
                    var _imgURL = data.Location.slice(_i);
                    //存储imgUrl
                    var index = +($(fileDom).attr('num'));
                    guiderImg_Arr[index] = _imgURL;
                    // console.log(studentCard_Arr);
                    // 需要清空上传控件
                    vm.guiderImg_imgArr = guiderImg_Arr;
                    clearInput('file');
                    vm.showLoading2 = false
                    vm.$toast('个人图片上传成功')
                    vm.is_upload_guiderImg = true
                }else{
                    vm.showLoading2 = false
                    vm.$toast('个人图片上传失败')
                    clearInput('file');
                    vm.is_upload_guiderImg = false
                }
            })
        }else{
            vm.showLoading2 = false
...
function set_COSImgKey(file) {
    ...
    if (data_type == 'card_1') { //身份证
        ...
        return _key;
    } else if (data_type == 'card_4') { //个人封面
        var _key = '/guiderImg/' + _customid + '_01' + _suffix;
        return _key;
    }
